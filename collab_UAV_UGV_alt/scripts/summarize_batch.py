#!/usr/bin/env python3
"""Aggregate a collab_UAV_UGV_alt batch into Table-3 metrics.

Usage:
  python collab_UAV_UGV_alt/scripts/summarize_batch.py <BATCH_DIR>

Scans <BATCH_DIR>/collab_<episode>_*/result.json (15 scenes x N runs),
computes SR_UAV / SR_UGV / SR_joint (union) / CG / Time, and writes
results.csv + summary.txt into <BATCH_DIR>.
...
def main():
    ...
    for r in results:
        ...
        rows.append({
            ...
            "joint": us or gs,
            "time_s": d.get("time_s"),
            "succ_time_s": d.get("success_time_s"),
    ...
    summary = (
        "============================================\n"
        f"  Table-3 Summary ({batch.name})\n"
        f"  Runs: {n}\n"
        f"  SR_UAV:   {sr_uav:.1f}%  ({n_uav}/{n})\n"
        f"  SR_UGV:   {sr_ugv:.1f}%  ({n_ugv}/{n})\n"
        f"  SR_joint: {sr_joint:.1f}%  ({n_joint}/{n})  [union]\n"
        f"  CG:       {cg:.1f}%\n"
        f"  Time (first-arrival): {fmt_mean_std(succ_times)} s\n"
        f"  Wall time (all):      {fmt_mean_std(all_times)} s\n"
        "--------------------------------------------\n"
        + "\n".join(scene_lines)
        + "\n============================================"
    )
    ...
    with csv_path.open("w", newline="") as f:
        ...
        w.writerow(["ep", "run", "uav_success", "ugv_success", "joint",
                    "time_s", "success_time_s", "uav_final_dist_m", "ugv_final_dist_m", "total_vlm_calls"])
        for r in rows:
            w.writerow([r["ep"], r["run"], r["uav_success"], r["ugv_success"],
                        r["joint"], r["time_s"], r["succ_time_s"], r["uav_dist"], r["ugv_dist"], r["vlm"]])
