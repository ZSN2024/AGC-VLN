...
#
# Usage:
#   bash collab_UAV_UGV_alt/scripts/auto_run_collab.sh [N] [EPISODE_ID]
#
# Flow per run:
...
SCRIPTS="$DIR/collab_UAV_UGV_alt/scripts"
TEMPLATES="$DIR/collab_UAV_UGV_alt/episodes/town10hd_templates.json"
RUNS="$DIR/collab_UAV_UGV_alt/runs"
# Reuse an existing batch dir (set by an outer N-scene loop) or create a new one.
BATCH_DIR=${BATCH_DIR:-"$RUNS/batch_$(date +%Y%m%d_%H%M%S)"}
mkdir -p "$BATCH_DIR"

echo "============================================"
echo "  Collab UAV+UGV Auto-Run: $N runs on $EP @ ${ALT}m"
...
python "$SCRIPTS/setup_scene.py" --episode-id "$EP" --episode-file "$TEMPLATES" --altitude "$ALT" $SETUP_EXTRA &
SCENE_PID=$!
sleep 6
...
SUCCESS=0; FAIL=0
UAV_SUCC=0; UGV_SUCC=0; JOINT_SUCC=0
...
for i in $(seq 1 $N); do
    echo ""
    echo "--- Run $i/$N ---"
    if python "$SCRIPTS/run_collab.py" --episode-id "$EP" --auto --time-limit 180 --run-dir "$BATCH_DIR"; then
    ...
    if [ -f "$RDIR/result.json" ]; then
        ...
        if [ "$us" = "True" ] || [ "$gs" = "True" ]; then JOINT_SUCC=$((JOINT_SUCC + 1)); fi
...
JOINT_RATE=$(awk "BEGIN {printf \"%.0f\", $JOINT_SUCC*100/$TOTAL}")
CG=$(awk "BEGIN {jr=$JOINT_SUCC*100.0/$TOTAL; m=($UAV_SUCC>$UGV_SUCC?$UAV_SUCC:$UGV_SUCC)*100.0/$TOTAL; printf \"%.1f\", jr-m}")
