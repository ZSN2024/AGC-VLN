...
# on the shared bird's-eye image. Progressive from raw to full:
#   none     → raw bird's-eye, no markers
#   goal     → + GOAL marker
#   car_goal → + CAR marker
#   line     → + CAR→GOAL reference line
#   full     → + distance labels & drone crosshair (original behavior)
ANNOTATION_LEVELS = {
    "none":     {"goal": False, "car": False, "line": False, "labels": False, "crosshair": False},
    "goal":     {"goal": True,  "car": False, "line": False, "labels": False, "crosshair": False},
    "car_goal": {"goal": True,  "car": True,  "line": False, "labels": False, "crosshair": False},
    "line":     {"goal": True,  "car": True,  "line": True,  "labels": False, "crosshair": False},
    "full":     {"goal": True,  "car": True,  "line": True,  "labels": True,  "crosshair": True},
}
...
def uav_loop(state: SharedState, air, client, cfg, ep, offset, log_dir, log_f, auto_mode, sim_cam=None, sim_frame=None, annot_level="full"):
    """
    ...
    """
    ann = ANNOTATION_LEVELS[annot_level]
    goal = ep["goal"]
    ...
    try:
        while not state.stop_event.is_set():
            ...
            if now >= next_decision:
                ...
                # Draw annotations on birdview according to the richness level
                if ann["crosshair"]:
...
def main():
    ...
    p.add_argument("--annotation", default="full", choices=list(ANNOTATION_LEVELS.keys()),
                   help="Map annotation richness level (Table 4 ablation)")
    args = p.parse_args()
    ...
    print(f"  Annotation richness: {args.annotation}")
    ...
    uav_th = threading.Thread(
        target=uav_loop,
        args=(state, air, client, cfg, ep, offset, log_dir, log_f, args.auto, sim_cam, sim_frame, args.annotation),
